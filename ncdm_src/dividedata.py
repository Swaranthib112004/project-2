...
# FastRBTree is not a standard Python library.
# For simplicity and to make the provided code runnable,
# we'll replace FastRBTree with a standard dictionary.
# If you have a specific FastRBTree implementation, you can re-integrate it.
...
def group_trainset_by_exercise():
    ...
    :return:
    '''
    exers = SimpleDictWrapper() # Replaced FastRBTree
...
def group_trainset_by_knowledge():
    with open('data/group_trainset_by_exercise.json', encoding='utf8') as i_f:
        exers = json.load(i_f)
    exers_rbt = SimpleDictWrapper() # Replaced FastRBTree
    for exer in exers:
        exers_rbt.insert(exer['exer_id'], exer)
    kn_rbt = SimpleDictWrapper() # Replaced FastRBTree
    for exer in exers:
...
if __name__ == '__main__':
    divide_data()
    # group_trainset_by_exercise() # Commented out as in original
    # group_trainset_by_knowledge() # Commented out as in original